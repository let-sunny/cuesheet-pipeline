...
cuesheet-draft assemble \
  ...
  --out <큐시트경로>.json \
  [--fps 30] [--width 1280] [--height 720] [--boundary-pad 0.4]
```
...
[
  {
    ...
    "monotonousRanges": [
      { "startS": 10, "endS": 55, "desc": "손으로 계속 뜨는 중", "faceExposed": false }
    ]
  }
]
```
...
- `monotonousRanges[].faceExposed`: 이 구간에 얼굴 노출 위험(턱끝 초과 노출)이 있는지 —
  비전 판독자는 **항상 명시할 것을 권장**한다. 생략하면 assemble이 `desc` 텍스트에서
  "얼굴"과 "노출"이 동시에 있는지로 보수적으로 폴백 판정한다(애매하면 위험 쪽으로).
...
- 정속 하이라이트: `quality >= 3`만 채택, `speed=1` `volume=1`, `subtitle=memo`.
  **경계 패딩**: 채택 시 `in = max(0, inS - PAD)`, `out = min(clipDur, outS + PAD)`를 먼저
  적용한다(`PAD` 기본 0.4초, `assembleDraft({ boundaryPadS })` 또는 CLI `--boundary-pad`로
  조절/비활성화 가능). 동작(뜨개 손짓)이 완성되기 전에 컷이 끊기는 문제를 막기 위한 여유다
  (Vrew "호흡 안 지킴" 불만, Descript 'Avoid harsh cuts' 참고). `clipDur`은 manifest.json의
  `durS`에서 가져오며(CLI가 자동 배선), 없는 클립은 상한 클램프를 생략한다. 같은 클립 내
  인접 컷끼리 패딩으로 겹치면 겹친 폭만큼만 반씩 되돌려 겹치지 않게 한다.
  개별 길이는 2~3.5초로 — 패딩 포함 길이가 3.5초를 넘으면 **양끝을 대칭으로 줄여**
  (동작 중심 유지) 3.5초로 클램프한다. 전체 정속 컷 평균이 3.1초를 넘으면 가장 긴 컷부터
  0.25초씩 양끝 대칭으로 다듬어 평균을 2.8~3.0초로 수렴시키는 그리디 패스를 한 번 돈다
  (사용자 실측 리듬 평균 2.95초 기준, 배속 커넥터는 건드리지 않음).
- 배속 커넥터: 같은 클립의 `monotonousRanges` 중 길이 30초 이상인 구간에서 30~60초
  ...
  에피소드당 최대 8개 상한(남발 방지). **얼굴 노출 위험 구간(`faceExposed: true` 또는
  desc 휴리스틱상 위험)은 커넥터로 선택하지 않는다** — 같은 클립에 안전한 단조구간이
  없으면 그 클립엔 커넥터를 넣지 않고 조용히 넘어간다(콘솔에 한 줄 로그만 남김).
