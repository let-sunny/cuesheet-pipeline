...
/** 세그먼트 블록 최소 폭(px) — 짧은 세그먼트도 눌러 선택할 수 있게 보장. */
 ...
 * 비례 폭으로 보여주고, 클릭으로 선택, 우측에 총 길이를 표시한다.
 * TimelineView(풀버전, 마무리 단계 전용)와는 별개의 얇은 전용 컴포넌트.
 */
export function MiniTimelineStrip({ segments, selectedIndex, onSelect }: Props) {
        ...
        {segments.map((seg, i) => {
          ...
          return (
            <button
              ...
              onClick={() => onSelect(i)}
              title={`${i + 1}. ${seg.clip || "(파일명 없음)"} · ${seg.in.toFixed(1)}s~${seg.out.toFixed(1)}s`}
            />
          );
        })}
      </div>
