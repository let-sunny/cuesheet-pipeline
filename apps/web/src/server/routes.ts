...
function formatMinSec(atS: number): string {
...
export function registerRoutes(
  ...
  // Captures a single full-resolution frame from the ORIGINAL clip (not the 720p preview proxy —
  // thumbnail candidates need source pixels) via seek-based ffmpeg (-ss before -i, fast even on a
  // long clip). /api/frame-capture?clip=<name>&atS=<source-seconds>
