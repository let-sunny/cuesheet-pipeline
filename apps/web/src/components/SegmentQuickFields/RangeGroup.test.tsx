...
describe("RangeGroup", () => {
  it("shows the clip filename read-only (as a title-carrying span, not an input)", () => {
    render(<RangeGroup clip="cut_01.mp4" lengthS={7} inField={numericField("2")} outField={numericField("9")} />);
    expect(screen.getByTitle("cut_01.mp4")).not.toBeNull();
  });

  it("shows the computed length", () => {
    render(<RangeGroup clip="a.mp4" lengthS={7} inField={numericField("2")} outField={numericField("9")} />);
